;Revision 04/01/2021
;Creator - Naveen
;I am Heimdall the Gatekeeper a servant of all New. I am the guardian of this machine, serving, protecting & denying access to any unauthorised beings.
#Persistent
...
^!z::
{
	...
	GoogleTranslate(Source)
	{
		...
		IE.Navigate(Path)
		;While (IE.document.readyState!="complete" || IE.busy)
				;Sleep 500
		;Result := IE.document.all.result_box.innertext
		
		SleepTime := StrLen(Source) * 10
		While (IE.busy)
		  Sleep, %SleepTime%
...
;ComObjCreate("SAPI.SpVoice").Speak("Hello Naveen Tell me what you need")
	InputBox, UserInput, Heimdall, `n Hello Naveen....Tell me what you need,  , 300,150,,  , 60
	if Errorlevel
		sleep 1
	...
	else
	{
		...
		if(Words0=1)
		{
					...
					run, C:\Users\zjjzrd\Documents\Trainings\Aptiv TCI Leave Calendar 2021.pdf
					
					else if UserInput contains jira
					run, C:\Program Files (x86)\Google\Chrome\Application\chrome.exe "https://jira-it.zone2.agileci.conti.de/secure/RapidBoard.jspa?rapidView=6669&projectKey=ASIT&quickFilter=26017"
					
					else if UserInput contains who
					MsgBox, ,Heimdall, I am Heimdall the Gatekeeper a servant of all New. I am the guardian of this machine here to serve and protect this device.
					...
					MsgBox, ,Heimdall, %A_DDDD%, %A_DD%/%A_MM%/%A_YYYY% ,  week : %Week% , Day of the year : %A_YDAY%
					}
...
;}
					...
					else if(UserInput="info")
					{
						...
						InputBox, InfoInput, Heimdall, `n Tell me what should I remember for you,  , 300,150,,  , 60
			...
			else
			{
				Loop
				{
					...
					if (UserInput=variable)
					{
						...
						op := op . "`n" . SubStr(line, FoundPos+1, StrLen(line))
...
;break
				...
				MsgBox, 262144, Heimdall, %UserInput% :: %op%
				op=
			}
			return
		}
...
;MsgBox, , Heimdall, %UserInput%
				...
				return
					...
					{
						Output := "Hmmm... I will think about it"
						MsgBox, , Heimdall, %Output% 
					}
